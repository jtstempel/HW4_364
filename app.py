...
## Ava Weiner helped me tinker with the register.html template, since I could not register new users in localhost - and with rendering GifSearchForm() 
## I also consulted https://stackoverflow.com/questions/21764770/typeerror-got-multiple-values-for-argument/21765286
...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/jstempelHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('tags', db.Column('search_term_id', db.Integer, db.ForeignKey('search_terms.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))
...
class User(UserMixin, db.Model):
    ...
    my_collections = db.relationship('PersonalGifCollection', backref = 'User')
...
def get_gifs_from_giphy(search_string):
    ...
    my_params = {'api_key': api_key, 'q': search_string, 'limit': 5}
    response = requests.get(baseurl, params = my_params)
    my_json = json.loads(response.text)['data']
    list_gifs = []
    for individual_gif in my_json:
        list_gifs.append(individual_gif)
    return list_gifs
...
def get_or_create_gif(db_session, title, url):
    ...
    my_gif = db_session.query(Gif).filter_by(title = title).first()
    if my_gif:
        return my_gif
    else:
        my_gif = Gif(title = title, embedURL = url)
        db_session.add(my_gif)
        db_session.commit()
        return my_gif
...
def get_or_create_search_term(db_session, term):
    ...
    search_term_instance = db_session.query(SearchTerm).filter_by(term = term).first()

    if search_term_instance:
        return search_term_instance

    else:
        search_term_instance = SearchTerm(term = term)
        my_search = get_gifs_from_giphy(term)

        for list_item in my_search:
            my_gif = get_or_create_gif(db_session, title = list_item['title'], url = list_item['embed_url'])
            search_term_instance.gifs.append(my_gif)
        
        db.session.add(search_term_instance)
        db.session.commit()
        return search_term_instance
...
def get_or_create_collection(db_session, name, current_user, gif_list=[]):
    ...
    my_gif_collection = PersonalGifCollection.query.filter_by(name = name, user_id = current_user.id).first()

    if my_gif_collection:
        return my_gif_collection

    else:
        my_gif_collection = PersonalGifCollection(name = name, user_id = current_user.id, gifs = [])
        for a_gif in gif_list:
            my_gif_collection.gifs.append(a_gif)

        db.session.add(my_gif_collection)
        db.session.commit()
...
def create_collection():
    ...
    if request.method == 'POST':
        for gif_id in form.gif_picks.data:
            get_gif_by_id(gif_id)

        my_collection = get_or_create_collection(db.session, name = form.name.data, current_user = current_user, gif_list = gifs)
...
def collections():
    ...
    each_collection = PersonalGifCollection.query.filter_by(user_id = current_user.id).all()
    return render_template('collections.html', collections = each_collection)
